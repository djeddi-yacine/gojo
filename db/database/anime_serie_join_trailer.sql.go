...
// versions:
//   sqlc v1.23.0
// source: anime_serie_join_trailer.sql
...
const getAnimeSerieTrailersDirectly = `-- name: GetAnimeSerieTrailersDirectly :many
...
func (q *Queries) GetAnimeSerieTrailersDirectly(ctx context.Context, animeID int64) ([]AnimeTrailer, error) {
	rows, err := q.db.Query(ctx, getAnimeSerieTrailersDirectly, animeID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
