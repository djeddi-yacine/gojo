// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.25.0
// source: anime_season.sql

package db

import (
	"context"
	"time"

	"github.com/jackc/pgx/v5/pgtype"
)

const createAnimeSeason = `-- name: CreateAnimeSeason :one
INSERT INTO anime_seasons (
    anime_id,
    season_original_title,
    release_year,
    aired,
    rating,
    portrait_poster,
    portrait_blur_hash,
    show_type
)
VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
RETURNING id, anime_id, season_original_title, release_year, aired, portrait_poster, portrait_blur_hash, rating, show_type, created_at
`

type CreateAnimeSeasonParams struct {
	AnimeID             int64
	SeasonOriginalTitle string
	ReleaseYear         int32
	Aired               time.Time
	Rating              string
	PortraitPoster      string
	PortraitBlurHash    string
	ShowType            string
}

func (q *Queries) CreateAnimeSeason(ctx context.Context, arg CreateAnimeSeasonParams) (AnimeSeason, error) {
	row := q.db.QueryRow(ctx, createAnimeSeason,
		arg.AnimeID,
		arg.SeasonOriginalTitle,
		arg.ReleaseYear,
		arg.Aired,
		arg.Rating,
		arg.PortraitPoster,
		arg.PortraitBlurHash,
		arg.ShowType,
	)
	var i AnimeSeason
	err := row.Scan(
		&i.ID,
		&i.AnimeID,
		&i.SeasonOriginalTitle,
		&i.ReleaseYear,
		&i.Aired,
		&i.PortraitPoster,
		&i.PortraitBlurHash,
		&i.Rating,
		&i.ShowType,
		&i.CreatedAt,
	)
	return i, err
}

const deleteAnimeSeason = `-- name: DeleteAnimeSeason :exec
DELETE FROM anime_seasons
WHERE id = $1
`

func (q *Queries) DeleteAnimeSeason(ctx context.Context, id int64) error {
	_, err := q.db.Exec(ctx, deleteAnimeSeason, id)
	return err
}

const getAnimeSeason = `-- name: GetAnimeSeason :one
SELECT id, anime_id, season_original_title, release_year, aired, portrait_poster, portrait_blur_hash, rating, show_type, created_at FROM anime_seasons
WHERE id = $1
LIMIT 1
`

func (q *Queries) GetAnimeSeason(ctx context.Context, id int64) (AnimeSeason, error) {
	row := q.db.QueryRow(ctx, getAnimeSeason, id)
	var i AnimeSeason
	err := row.Scan(
		&i.ID,
		&i.AnimeID,
		&i.SeasonOriginalTitle,
		&i.ReleaseYear,
		&i.Aired,
		&i.PortraitPoster,
		&i.PortraitBlurHash,
		&i.Rating,
		&i.ShowType,
		&i.CreatedAt,
	)
	return i, err
}

const updateAnimeSeason = `-- name: UpdateAnimeSeason :one
UPDATE anime_seasons
SET
  season_original_title = COALESCE($1, season_original_title),
  release_year = COALESCE($2, release_year),
  aired = COALESCE($3, aired),
  rating = COALESCE($4, rating),
  portrait_poster = COALESCE($5, portrait_poster),
  portrait_blur_hash = COALESCE($6, portrait_blur_hash),
  show_type = COALESCE($7, show_type)
WHERE
  id = $8
RETURNING id, anime_id, season_original_title, release_year, aired, portrait_poster, portrait_blur_hash, rating, show_type, created_at
`

type UpdateAnimeSeasonParams struct {
	SeasonOriginalTitle pgtype.Text
	ReleaseYear         pgtype.Int4
	Aired               pgtype.Timestamptz
	Rating              pgtype.Text
	PortraitPoster      pgtype.Text
	PortraitBlurHash    pgtype.Text
	ShowType            pgtype.Text
	ID                  int64
}

func (q *Queries) UpdateAnimeSeason(ctx context.Context, arg UpdateAnimeSeasonParams) (AnimeSeason, error) {
	row := q.db.QueryRow(ctx, updateAnimeSeason,
		arg.SeasonOriginalTitle,
		arg.ReleaseYear,
		arg.Aired,
		arg.Rating,
		arg.PortraitPoster,
		arg.PortraitBlurHash,
		arg.ShowType,
		arg.ID,
	)
	var i AnimeSeason
	err := row.Scan(
		&i.ID,
		&i.AnimeID,
		&i.SeasonOriginalTitle,
		&i.ReleaseYear,
		&i.Aired,
		&i.PortraitPoster,
		&i.PortraitBlurHash,
		&i.Rating,
		&i.ShowType,
		&i.CreatedAt,
	)
	return i, err
}
